...
function hasDll () {
  return config.custom.dll && config.custom.dll instanceof Array
}
exports.hasDll = hasDll
/**
 * banner 信息
 * @returns {string}
 */
...
function getThisChunkDllPath (customConfig) {
  var result = []
  getExtfiles(customConfig, 'js').forEach(function (d) {
    result.push(path.join(customConfig.dllOutput.linkPath, d))
  })
  return result
}
exports.getThisChunkDllPath = getThisChunkDllPath
function getExtfiles (customConfig, ext) {
  var filesname = fs.readdirSync(customConfig.dllOutput.absolutePath)
  return filesname.filter(function (d) {
    var _ext = d.split('.').pop()
    return _ext === ext
  })
}
exports.getExtfiles = getExtfiles
...
function lessLoader (styleHash) {
  var arr = [ 'postcss-loader', 'less-loader' ]
  if (styleHash) arr.unshift('css-loader?modules&importLoaders=1&localIdentName=[local]-[hash:base64:7]')
  else arr.unshift('css-loader')
  return arr.join('!')
}
